...
export default function ImageInfoPanel() {
  ...
  return (
    <div className="sidebar-panel flex flex-col gap-2.5 rounded-lg p-3 shadow-sm transition-all duration-200 hover:bg-card hover:shadow-md hover:border-border">
      {/* Collapsible header */}
      <button
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-foreground">Image Info</span>
        </div>
        <motion.div
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-[10px] pl-1">
                <span className="text-muted-foreground/70 font-medium">Dimensions</span>
                <span className="text-muted-foreground tabular-nums">{origW} × {origH}</span>

                <span className="text-muted-foreground/70 font-medium">Size</span>
                <span className="text-muted-foreground tabular-nums">{formatFileSize(origSize)}</span>

                <span className="text-muted-foreground/70 font-medium">Format</span>
                <span className="text-muted-foreground">{origFormat}</span>

                {originalImage.originalName && (
                  <>
                    <span className="text-muted-foreground/70 font-medium">Filename</span>
                    <span className="text-muted-foreground truncate max-w-[140px]">{originalImage.originalName}</span>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-semibold text-primary uppercase tracking-wider">After</span>
              </div>
              <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-[10px] pl-1">
                <span className="text-foreground/70 font-medium">Dimensions</span>
                <span className="text-foreground tabular-nums">{procW} × {procH}</span>

                <span className="text-foreground/70 font-medium">Size</span>
                <span className="text-foreground tabular-nums">{formatFileSize(procSize)}</span>

                <span className="text-foreground/70 font-medium">Format</span>
                <span className="text-foreground">{procFormat}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 border-t border-border/50 pt-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-semibold text-foreground uppercase tracking-wider">Comparison</span>
              </div>
              ...
              <div className="flex flex-col gap-1.5 pl-1">
                <div className="flex items-center justify-between text-[10px]">
                  ...
                  <span className={`font-semibold tabular-nums ${sizeIncrease ? 'text-red-500' : sizeIndicator.className}`}>
                ...
                <div className="flex items-center justify-between text-[9px]">
                  ...
                  <span className={`${sizeIndicator.className} font-medium`}>
                    {sizeIndicator.label}
                  </span>
              ...
              <div className="flex items-center justify-between text-[10px] pl-1">
                <span className="font-medium text-foreground/70">Dimensions</span>
              ...
              {/* Pixel diff stats */}
              <div className="flex items-center justify-between text-[10px] pl-1">
                <span className="font-medium text-foreground/70">Pixels modified</span>
                <span className="font-semibold tabular-nums text-foreground">
                  {diffLoading
                    ? 'Computing...'
                    : diffStats
